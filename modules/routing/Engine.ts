...
import type * as RoutingTypes from './Types';
...
type Route<Ctx, Res> = {
    path: string;
    segments: Segment[];
    handler: RoutingTypes.Ports.Handler<Ctx, Res>;
};
...
function composeMiddleware<Ctx, Res>(
    middlewares: RoutingTypes.Ports.Middleware<Ctx, Res>[],
    handler: RoutingTypes.Ports.Handler<Ctx, Res>
): RoutingTypes.Ports.Handler<Ctx, Res> {
    return middlewares.reduceRight<RoutingTypes.Ports.Handler<Ctx, Res>>(
        (next, mw) => (ctx: Ctx) => mw(ctx, () => next(ctx)),
        handler
    );
}

export function create<Ctx = any, Res = any>(
    logger?: RoutingTypes.Ports.Logger
): RoutingTypes.Router<Ctx, Res> {
    const middlewares: RoutingTypes.Ports.Middleware<Ctx, Res>[] = [];
    const routes: Route<Ctx, Res>[] = [];
    const log = logger ?? { info: (_: string) => {}, error: (_: string) => {} };

    function use(mw: RoutingTypes.Ports.Middleware<Ctx, Res>): RoutingTypes.Router<Ctx, Res> {
        middlewares.push(mw);
        return api;
    }
    ...
    function registerAll(map: {
        [path: string]: RoutingTypes.Ports.Handler<Ctx, Res>;
    }): RoutingTypes.Router<Ctx, Res> {
    ...
    function mount(
        prefix: string,
        sub: RoutingTypes.Router<Ctx, Res>
    ): RoutingTypes.Router<Ctx, Res> {
        // Resolve sub routes by wrapping dispatch
        return register(prefix + '/*', (ctx: any) =>
            (sub as any).dispatch('/' + ctx.params['*'], ctx)
        );
    }

    function resolve(
        path: string
    ): { handler: RoutingTypes.Ports.Handler<Ctx, Res>; params: any } | null {
    ...
    const api: RoutingTypes.Router<Ctx, Res> = {
        use,
        register,
        registerAll,
        mount,
