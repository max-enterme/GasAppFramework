...
Test.it('GAS ScriptApp trigger management', () => {
    ...
    try {
        ...
        Assert.notNull(trigger, 'Trigger should be created');
        ...
        Assert.equals(mockScriptApp.getScriptTriggers().length, 2, 'Should have 2 triggers');

        // Test: Delete trigger
        mockScriptApp.deleteTrigger(trigger);
        Assert.equals(mockScriptApp.getScriptTriggers().length, 1, 'Should have 1 trigger after deletion');

        const remainingTrigger = mockScriptApp.getScriptTriggers()[0];
        Assert.equals(remainingTrigger.getHandlerFunction(), 'hourlyCheck', 'Remaining trigger should be hourly');
...
Test.it('GAS PropertiesService for application configuration', () => {
    ...
    try {
        ...
        Assert.equals(scriptProps.getProperty('app.version'), '1.2.3', 'Should store app version');
        Assert.equals(scriptProps.getProperty('app.environment'), 'production', 'Should store environment');
        ...
        Assert.equals(allProps['cache.ttl'], '3600', 'Batch properties should be set');
        Assert.equals(allProps['retry.attempts'], '3', 'Multiple properties should be set');

        // Test: User-specific properties
        userProps.setProperty('timezone', 'Europe/London');
        userProps.setProperty('notifications', 'enabled');

        Assert.equals(userProps.getProperty('timezone'), 'Europe/London', 'Should store user timezone');
        Assert.equals(userProps.getProperty('notifications'), 'enabled', 'Should store user preferences');

        // Test: Property deletion
        scriptProps.deleteProperty('database.url');
        Assert.isTrue(scriptProps.getProperty('database.url') === null, 'Deleted property should return null');
...
Test.it('GAS execution context and user information', () => {
    ...
    try {
        ...
        Assert.equals(mockSession.getScriptTimeZone(), 'America/New_York', 'Should have default timezone');

        mockSession.setTimeZone('Asia/Tokyo');
        Assert.equals(mockSession.getScriptTimeZone(), 'Asia/Tokyo', 'Should update timezone');

        // Test: Active user information
        const activeUser = mockSession.getActiveUser();
        Assert.equals(activeUser.getEmail(), 'test@example.com', 'Should have active user email');

        mockSession.setUserEmail('admin@company.com');
        Assert.equals(mockSession.getActiveUser().getEmail(), 'admin@company.com', 'Should update user email');
...
Test.it('GAS error handling and logging integration', () => {
    ...
    try {
        ...
        Assert.isTrue(!!lastLog, 'Error should be logged');
        Assert.isTrue(lastLog!.includes('Test error message'), 'Log should contain error message');
        Assert.isTrue(lastLog!.includes('testFunction'), 'Log should contain context');
        ...
        Assert.isTrue(mockLogger.getLastLog()!.includes('data-processing completed in 1234ms'), 'Should log performance');
...
Test.it('GAS quota and execution limit handling', () => {
    ...
    try {
        ...
        Assert.isTrue(fastResult, 'Fast operation should succeed');

        // Test: Use Utilities.sleep for pacing
        mockUtilities.sleep(100);
        Assert.equals(mockUtilities.getLastSleepDuration(), 100, 'Should record sleep duration');
        ...
        Assert.equals(processed.length, 10, 'Should process all items');
        Assert.equals(processed[0], 2, 'Items should be processed correctly');

        const logs = mockLogger.getAllLogs();
        Assert.isTrue(logs.some(log => log.includes('Processed batch 1')), 'Should log batch progress');
...
Test.it('GAS trigger-based workflow execution', () => {
    ...
    try {
        ...
        Assert.equals(workflowTrigger.getHandlerFunction(), 'executeWorkflow', 'Trigger should target workflow function');

        // Test: Execute workflow (simulate trigger firing)
        (globalThis as any).executeWorkflow();

        // Verify workflow execution
        Assert.equals(workflowState.executedTasks.length, 3, 'Should execute all pending tasks');
        Assert.isTrue(workflowState.executedTasks.includes('task1'), 'Should execute task1');
        Assert.isTrue(workflowState.executedTasks.includes('task2'), 'Should execute task2');
        Assert.isTrue(workflowState.executedTasks.includes('task3'), 'Should execute task3');

        const logs = mockLogger.getAllLogs();
        Assert.isTrue(logs.some(log => log.includes('Workflow execution started')), 'Should log workflow start');
        Assert.isTrue(logs.some(log => log.includes('Executing task: Daily Report')), 'Should log task execution');
        Assert.isTrue(logs.some(log => log.includes('Workflow completed')), 'Should log workflow completion');
...
Test.it('Complete GAS application lifecycle management', () => {
    ...
    try {
        ...
        Assert.isTrue(initResult.success, 'Application should initialize successfully');
        Assert.equals(initResult.version, '1.0.0', 'Should return correct version');
        ...
        Assert.isTrue(!!user1Result.requestId, 'Should process user 1 request');
        Assert.isTrue(!!user2Result.requestId, 'Should process user 2 request');
        Assert.equals(appState.activeUsers.size, 2, 'Should track 2 active users');
        ...
        Assert.equals(cleanupTrigger.getHandlerFunction(), 'cleanupApplication', 'Cleanup trigger should be set');
        ...
        Assert.equals(cleanupResult.activeUsers, 2, 'Cleanup should report active user count');
        Assert.isTrue(cleanupResult.uptime > 0, 'Should report positive uptime');

        // Verify logging throughout lifecycle
        const logs = mockLogger.getAllLogs();
        Assert.isTrue(logs.some(log => log.includes('Initializing application')), 'Should log initialization');
        Assert.isTrue(logs.some(log => log.includes('Handling request from user: alice@example.com')), 'Should log user requests');
        Assert.isTrue(logs.some(log => log.includes('Application cleanup completed')), 'Should log cleanup');

        // Verify properties were set
        const props = (globalThis as any).PropertiesService.getScriptProperties();
        Assert.equals(props.getProperty('app.initialized'), 'true', 'Should set initialized flag');
        Assert.equals(props.getProperty('app.version'), '1.0.0', 'Should store version');
        Assert.isTrue(!!props.getProperty('app.shutdown'), 'Should record shutdown time');
