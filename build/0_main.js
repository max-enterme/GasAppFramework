...
// NAMESPACE OBJECT: ./modules/index.ts
...
__webpack_require__.d(modules_namespaceObject, {
  ...
  GasLogger: () => (GasLogger),
  Inject: () => (Inject),
  Locking: () => (locking_namespaceObject),
  MemoryStore: () => (MemoryStore),
  PropertiesStore: () => (PropertiesStore),
  RepoError: () => (RepositoryError),
  Repository: () => (repository_namespaceObject),
  Resolve: () => (Resolve),
  RestFramework: () => (rest_framework_namespaceObject),
  Routing: () => (routing_namespaceObject),
  Shared: () => (shared_namespaceObject),
  SpreadsheetStore: () => (SpreadsheetStore),
  StringHelper: () => (string_helper_namespaceObject),
  SystemClock: () => (SystemClock),
  TestRunner: () => (test_runner_namespaceObject),
  Testing: () => (testing_namespaceObject),
  ensureTimeZone: () => (ensureTimeZone),
  formatDate: () => (format)
});
...
(function (GAS) {
    ...
    GAS.MockSpreadsheetApp = MockSpreadsheetApp;
    ...
    class MockSheet {
        ...
        getDataRange() {
            var _a;
            return new MockRange(this.data, 1, 1, this.data.length, ((_a = this.data[0]) === null || _a === void 0 ? void 0 : _a.length) || 0);
        }
        ...
        getLastColumn() {
            var _a;
            return ((_a = this.data[0]) === null || _a === void 0 ? void 0 : _a.length) || 0;
        }
    ...
    class MockRange {
        constructor(sheetData, row, col, numRows, numCols) {
        ...
        getValues() {
            ...
            for (let i = 0; i < this.numRows; i++) {
                ...
                for (let j = 0; j < this.numCols; j++) {
                    ...
                    rowData.push((_b = (_a = this.sheetData[rowIndex]) === null || _a === void 0 ? void 0 : _a[colIndex]) !== null && _b !== void 0 ? _b : null);
                }
                result.push(rowData);
            }
            return result;
        ...
        getValue() {
            ...
            return (_b = (_a = this.sheetData[rowIndex]) === null || _a === void 0 ? void 0 : _a[colIndex]) !== null && _b !== void 0 ? _b : null;
        }
        setValue(value) {
            const rowIndex = this.row - 1;
            const colIndex = this.col - 1;
    ...
    GAS.MockPropertiesService = MockPropertiesService;
    ...
    GAS.MockLockService = MockLockService;
    class MockLock {
        constructor(id) {
            this.id = id;
            this.acquired = false;
    ...
    GAS.MockScriptApp = MockScriptApp;
    class MockTriggerBuilder {
        constructor(functionName, scriptApp) {
    ...
    GAS.MockUtilities = MockUtilities;
    // Global mock instances
    let mockSpreadsheetApp = null;
    let mockPropertiesService = null;
    let mockLockService = null;
    let mockScriptApp = null;
    let mockUtilities = null;
    /**
    ...
    function installAll() {
        ...
        globalThis.LockService = mockLockService;
        globalThis.ScriptApp = mockScriptApp;
    ...
    GAS.installAll = installAll;
    /**
     * Reset all GAS mocks to initial state
     */
    function resetAll() {
        mockSpreadsheetApp === null || mockSpreadsheetApp === void 0 ? void 0 : mockSpreadsheetApp.reset();
        mockPropertiesService === null || mockPropertiesService === void 0 ? void 0 : mockPropertiesService.reset();
        mockLockService === null || mockLockService === void 0 ? void 0 : mockLockService.reset();
        mockScriptApp === null || mockScriptApp === void 0 ? void 0 : mockScriptApp.reset();
...
function initializeGasAppFramework() {
    ...
    const logger = (typeof Logger !== 'undefined') ? Logger : console;
    logger.log('[INIT] Framework.Testing.TestHelpers type: ' + typeof TestHelpers_namespaceObject);
    logger.log('[INIT] Framework.Testing.TestHelpers keys: ' + Object.keys(TestHelpers_namespaceObject).join(', '));
    if (GAS) {
        logger.log('[INIT] Framework.Testing.TestHelpers.GAS type: ' + typeof GAS);
        logger.log('[INIT] Framework.Testing.TestHelpers.GAS.installAll type: ' + typeof GAS.installAll);
    }
    // Export framework and testing globals
    // These will be available when GAS loads test files
    globalThis.GasAppFramework = modules_namespaceObject;
    globalThis.T = Test_namespaceObject;
    globalThis.TRunner = Runner_namespaceObject;
    globalThis.TAssert = Assert_namespaceObject;
    globalThis.TGasReporter = GasReporter_namespaceObject;
    // Export TestHelpers namespace
    globalThis.TestHelpers = TestHelpers_namespaceObject;
    // Debug: Check after assignment
    logger.log('[INIT] globalThis.TestHelpers type: ' + typeof globalThis.TestHelpers);
    logger.log('[INIT] globalThis.TestHelpers.GAS type: ' + typeof globalThis.TestHelpers.GAS);
    // Export common modules for convenience
    logger.log('[DEBUG] Framework.MemoryStore: ' + typeof MemoryStore);
    logger.log('[DEBUG] Framework.SpreadsheetStore: ' + typeof SpreadsheetStore);
    logger.log('[DEBUG] Framework.PropertiesStore: ' + typeof PropertiesStore);
    const repository = Object.assign({}, repository_namespaceObject);
    repository.MemoryStore = MemoryStore;
    repository.SpreadsheetStore = SpreadsheetStore;
    globalThis.Repository = repository;
    const locking = Object.assign({}, locking_namespaceObject);
    locking.PropertiesStore = PropertiesStore;
    locking.SystemClock = SystemClock;
    locking.GasLogger = GasLogger;
    globalThis.Locking = locking;
    globalThis.Routing = routing_namespaceObject;
    globalThis.StringHelper = string_helper_namespaceObject;
    globalThis.Shared = shared_namespaceObject;
    // Debug: Check exports structure
    logger.log('[DEBUG] globalThis.Locking.PropertiesStore: ' + typeof globalThis.Locking.PropertiesStore);
    logger.log('[DEBUG] globalThis.Repository.MemoryStore: ' + typeof globalThis.Repository.MemoryStore);
    logger.log('[DEBUG] globalThis.Repository.SpreadsheetStore: ' + typeof globalThis.Repository.SpreadsheetStore);
...
function debugTestHelpers() {
    ...
    logger.log('typeof TestHelpers: ' + typeof globalThis.TestHelpers);
    logger.log('typeof Framework.Testing.TestHelpers: ' + typeof TestHelpers_namespaceObject);
    if (typeof globalThis.TestHelpers !== 'undefined') {
        logger.log('TestHelpers keys: ' + Object.keys(globalThis.TestHelpers).join(', '));
        logger.log('typeof TestHelpers.GAS: ' + typeof globalThis.TestHelpers.GAS);
        if (typeof globalThis.TestHelpers.GAS !== 'undefined') {
            logger.log('TestHelpers.GAS keys: ' + Object.keys(globalThis.TestHelpers.GAS).join(', '));
        }
    }
    logger.log('=========================================');
}
